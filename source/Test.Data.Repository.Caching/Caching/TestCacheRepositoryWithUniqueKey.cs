...
{
    /// <summary>
    /// Testing <see cref="CacheReadDataRepositoryWithUniqueKey{TEntity, TPrimaryKey, TUniqueKey}"/> and <see cref="CacheWriteDataRepository{TEntity, TKey}"/>.
    /// </summary>
    [Collection("Caching")]
    public class TestCacheRepositoryWithUniqueKey
    {
        public TestCacheRepositoryWithUniqueKey(ITestOutputHelper output)
        {
            m_Output = output;
        }
        ...
        public void AssertCacheLifetime()
        {
            ...
            var writeRepo = new SampleWriteRepository(database).WithCache(x => x.ID);
            var readRepo = new SampleReadRepositoryWithUniqueKey(database).WithCache(x => x.ID, x => x.Code!);

            m_Output.WriteLine("----- get entity -----");

            Assert.Null(readRepo.GetByPrimaryKey(1));
            Assert.Null(readRepo.GetByUniqueKey("001"));
            ...
            var found1 = readRepo.GetByPrimaryKey(1);
            ...
            Assert.Equal(found1, readRepo.GetByPrimaryKey(1));

            // The cached entity will be returned.
            Assert.Equal(found1, readRepo.GetByUniqueKey("001"));

            m_Output.WriteLine("----- get entities -----");

            // Only entity1, the cached entity will be returned.
            var founds = readRepo.GetRangeByPrimaryKey(new[] { 1, 2, 3 }).ToDictionary(x => x.ID);
            ...
            founds = readRepo.GetRangeByUniqueKey(new[] { "001", "002", "003" }).ToDictionary(x => x.ID);
            ...
            // The cached entity will be returned.
            Assert.Equal(founds[1], readRepo.GetByPrimaryKey(1));
            Assert.Equal(founds[2], readRepo.GetByPrimaryKey(2));
            Assert.Equal(founds[3], readRepo.GetByPrimaryKey(3));

            Assert.Equal(founds[1], readRepo.GetByUniqueKey("001"));
            Assert.Equal(founds[2], readRepo.GetByUniqueKey("002"));
            Assert.Equal(founds[3], readRepo.GetByUniqueKey("003"));
            ...
            found1 = readRepo.GetByPrimaryKey(1);

            Assert.Null(found1);

            found1 = readRepo.GetByUniqueKey("001");

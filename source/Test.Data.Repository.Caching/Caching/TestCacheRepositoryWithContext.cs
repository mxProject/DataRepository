...
{
    /// <summary>
    /// Testing <see cref="CacheReadDataRepositoryWithContext{TEntity, TKey, TContext}"/> and <see cref="CacheWriteDataRepositoryWithContext{TEntity, TKey, TContext}"/>.
    /// </summary>
    [Collection("Caching")]
    public class TestCacheRepositoryWithContext
    {
        public TestCacheRepositoryWithContext(ITestOutputHelper output)
        {
            m_Output = output;
        }
        ...
        public void AssertCacheLifetime()
        {
            ...
            var readRepo = new SampleReadRepositoryWithContext(database).WithCache(x => x.ID);

            m_Output.WriteLine("----- get entity -----");

            Assert.Null(readRepo.Get(1, context));
            ...
            var found1 = readRepo.Get(1, context);
            ...
            Assert.Equal(found1, readRepo.Get(1, context));

            m_Output.WriteLine("----- get entities -----");

            // Only entity1, the cached entity will be returned.
            var founds = readRepo.GetRange(new[] { 1, 2, 3 }, context).ToDictionary(x => x.ID);
            ...
            founds = readRepo.GetRange(new[] { 1, 2, 3 }, context).ToDictionary(x => x.ID);
            ...
            // The cached entity will be returned.
            Assert.Equal(founds[1], readRepo.Get(1, context));
            Assert.Equal(founds[2], readRepo.Get(2, context));
            Assert.Equal(founds[3], readRepo.Get(3, context));

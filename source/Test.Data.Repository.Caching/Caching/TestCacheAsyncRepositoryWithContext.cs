...
{
    /// <summary>
    /// Testing <see cref="CacheAsyncReadDataRepositoryWithContext{TEntity, TKey, TContext}"/> and <see cref="CacheAsyncWriteDataRepositoryWithContext{TEntity, TKey, TContext}"/>.
    /// </summary>
    [Collection("Caching")]
    public class TestCacheAsyncRepositoryWithContext
    {
        public TestCacheAsyncRepositoryWithContext(ITestOutputHelper output)
        {
            m_Output = output;
        }
        ...
        public async Task AssertCacheLifetimeAsync()
        {
            ...
            var readRepo = new SampleAsyncReadRepositoryWithContext(database).WithCache(x => x.ID);

            m_Output.WriteLine("----- get entity -----");

            Assert.Null(await readRepo.GetAsync(1, context));
            ...
            var found1 = await readRepo.GetAsync(1, context);
            ...
            Assert.Equal(found1, await readRepo.GetAsync(1, context));

            m_Output.WriteLine("----- get entities -----");

            // Only entity1, the cached entity will be returned.
            var founds = (await ToEnumerableAsync(readRepo.GetRangeAsync(new[] { 1, 2, 3 }, context))).ToDictionary(x => x.ID);
            ...
            founds = (await ToEnumerableAsync(readRepo.GetRangeAsync(new[] { 1, 2, 3 }, context))).ToDictionary(x => x.ID);
            ...
            // The cached entity will be returned.
            Assert.Equal(founds[1], await readRepo.GetAsync(1, context));
            Assert.Equal(founds[2], await readRepo.GetAsync(2, context));
            Assert.Equal(founds[3], await readRepo.GetAsync(3, context));
            ...
            found1 = await readRepo.GetAsync(1, context);

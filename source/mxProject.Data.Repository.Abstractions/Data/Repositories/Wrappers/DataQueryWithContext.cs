...
namespace mxProject.Data.Repositories.Wrappers
{
    ...
    public class DataQueryWithContext<TEntity, TCondition, TContext> : IDataQueryWithContext<TEntity, TCondition, TContext>
        where TContext : IDataRepositoryContext
    {
        ...
        public DataQueryWithContext(
            Func<bool> useTransactionScope,
            Action dispose,
            Func<TCondition, TContext, int> getCount,
            Func<TCondition, TContext, int, int?, IEnumerable<TEntity>> query
            )
        {
        ...
        private readonly Func<TCondition, TContext, int> m_GetCount;
        private readonly Func<TCondition, TContext, int, int?, IEnumerable<TEntity>> m_Query;
        ...
        {
            return m_Query(condition, context, skipCount, maximumCount);
        }
    }
}
